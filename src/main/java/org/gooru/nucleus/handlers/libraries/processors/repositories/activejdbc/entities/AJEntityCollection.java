...
public class AJEntityCollection extends Model {
  ...
  public static final String SELECT_COLLECTIONS =
      "SELECT id, title, course_id, publish_status, thumbnail, taxonomy, collaborator, visible_on_profile, learning_objective, owner_id,"
          + " original_creator_id FROM collection WHERE id = ANY(?::uuid[]) AND format = 'collection'::content_container_type AND"
          + " is_deleted = false ORDER BY updated_at DESC";

  public static final String SELECT_ASSESSMENTS =
      "SELECT id, title, course_id, publish_status, thumbnail, taxonomy, collaborator, visible_on_profile, learning_objective, owner_id,"
          + " original_creator_id FROM collection WHERE id = ANY(?::uuid[]) AND format = 'assessment'::content_container_type AND"
          + " is_deleted = false ORDER BY updated_at DESC";
  ...
  public static final List<String> COLLECTION_LIST =
      Arrays.asList(ID, TITLE, COURSE_ID, PUBLISH_STATUS, THUMBNAIL, TAXONOMY, COLLABORATOR,
          VISIBLE_ON_PROFILE, LEARNING_OBJECTIVE, OWNER_ID, ORIGINAL_CREATOR_ID);

  public static final List<String> ASSESSMENT_LIST =
      Arrays.asList(ID, TITLE, COURSE_ID, PUBLISH_STATUS, THUMBNAIL, TAXONOMY, COLLABORATOR,
          VISIBLE_ON_PROFILE, LEARNING_OBJECTIVE, OWNER_ID, ORIGINAL_CREATOR_ID);

  public static final List<String> OFFLINE_ACTIVITY_LIST = Arrays.asList(ID, TITLE, COURSE_ID,
      PUBLISH_STATUS, THUMBNAIL, TAXONOMY, COLLABORATOR, VISIBLE_ON_PROFILE, LEARNING_OBJECTIVE,
      OWNER_ID, ORIGINAL_CREATOR_ID, MAX_SCORE, DURATION_HOURS, REFERENCE, EXEMPLAR);
